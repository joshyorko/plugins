...
Record repair lineage on the admitted task: `attempt`, `progress`, `diagnosis`, and `next bounded action`. Increment `attempt` for every failed same-gap attempt, including a different worker or renamed task. Reset neither the count nor G when changing implementation subjects. A materially new observation may justify a new candidate, but it must not hide the original gap's history.

The finish decision is equally guarded: report `Converged(G)` only when every mandatory criterion is proven for the current subject and affected assumptions. If a mandatory gap remains and no authorized action can reduce it, report `unconverged/quiescent` with the exact blocker and resumption condition. Never infer convergence from an empty READY set, returned workers, an exhausted retry budget, or a clean unrelated check.
